...
type
  ...
  protected
    ...
    function MakeListNode(TV: TObject = nil): TObject; override;
    procedure DrawTreeNode(C: TCanvas; Left, Top: Integer); override;
...
begin
  ...
  I := 0;
  B := false;
  while not B and (I < GDbInput) do
  begin
    Input.AssignFromInt(I, L);
...
procedure TSConfig.MakeChildren;
  ...
  begin
    ...
    C.Create(Self, GetProgram.FExit, nil, FLetterInd, TESentence.Create);
...
function TSConfig.MakeListNode(TV: TObject): TObject;
begin
  if FLine is TETState then
    Result := inherited MakeListNode(TV)
  else
    Result := nil;
end;
...
//    exit;
  ...
  if W <> GetProgram.FEps then
    S.Pop;

  if not (D as TESentence).IsEps then
    S.Push(TESentence(D));

  if Re = GetProgram.FEps then
    I := FLetterInd
  else
    I := FLetterInd + 1;
...
procedure TSConfig.DrawTreeNode(C: TCanvas; Left, Top: Integer);
var
